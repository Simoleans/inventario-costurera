<template>
        ...
        <div class="overflow-y-auto scrolling-sticky" style="height: calc(100% - 72px)">
          <div class="flex justify-between items-center px-6 py-5">
            <div class="text-center">
              <h1 class="font-[500] text-[22px]">Editar Categoría</h1>
            </div>
  ...
  const isFormValid = computed(() => {
    return formData.name !== ''; // Verifica si ambos campos no están vacíos
  });
  ...
  const handleSave = () => {
    formData.put(route('category.update', formData.id), {
      preserveScroll: true,
      onSuccess: () => {
        closeModal();
        initializeForm();
