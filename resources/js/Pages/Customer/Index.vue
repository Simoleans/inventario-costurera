<template>
    <AppLayout title="Clientes">
        ...
        <Create :open="isSideMenuOpen" @close="closeCreateForm"/>
        <Edit :open="openEdit" @close="closeEditForm" :data="handleData"/>
        ...
        <Modal :show="isOpenDelete" @close="closeModalDelete">
            <div class="p-6">
                ...
                <p v-if="employe.del == 1" class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                    Una vez que se elimine el cliente, todos sus recursos y datos se eliminarán de forma permanente.
                </p>
                <p v-else class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                    Este cliente ya tiene datos asociados, no se puede eliminar.
                </p>
...
const cols = ref([
    { field: 'name_complete', title: 'Nombre' },
...
const handleDeleteData = () => {
    form.put(route('customer.updateStatus', form.id), {
        preserveScroll: true,
        onSuccess: () => {
            closeModalDelete();
