<template>
        ...
        <div class="overflow-y-auto scrolling-sticky" style="height: calc(100% - 72px)">
          <div class="flex justify-between items-center px-6 py-5">
            <div class="text-center">
              <h1 class="font-[500] text-[22px]">Editar Cliente</h1>
            </div>
            ...
            <form class="space-y-4">
              <div>
                ...
                <input
                  type="text"
                  id="name"
                  v-model="formData.name_complete"
  ...
  const formData = useForm({
    id: props.data.id,
    name_complete: props.data.name_complete,
  });

  watch(() => props.data, (newVal) => {
    formData.id = newVal.id;
    formData.name_complete = newVal.name_complete;
  });
  ...
  const isFormValid = computed(() => {
    return formData.name_complete !== ''; // Verifica si ambos campos no están vacíos
  });
  ...
  const handleSave = () => {
    formData.put(route('customer.update', formData.id), {
      preserveScroll: true,
      onSuccess: () => {
        closeModal();
        initializeForm();
